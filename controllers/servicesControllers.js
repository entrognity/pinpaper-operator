...
const { Services, ServicesPrices, ServicesTrackings, Pricings } = require('../models/servicesModel');
...
exports.updatePrices = async (req, res) => {
    ...
    try {
        ...
        // Prepare the update operation excluding accountID
        const { operatorID, ...updatePrices } = prices; // Destructure to exclude accountID

        // Update records based on accountID
        const result = await ServicesPrices.updateOne(
            { operatorID: operatorID },
            { $set: updatePrices } // Use updateFields for updating only relevant fields
        );

        if (result.nModified === 0) {
            return res.status(404).json({ error: 'No operator found' });
        }
